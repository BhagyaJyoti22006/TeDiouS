...
async def solve(request: Request):
    ...
    prompt = (
        "You are an expert mathematical solver. Read the following problem carefully. "
        "It deliberately contains DISTRACTOR numbers and TRICKY VOCABULARY designed to fool you.\n\n"
        "Follow these steps perfectly:\n"
        "1. Identify the core question. Pay intense attention to specific verbs (e.g., 'packed' vs 'held'). "
        "If a unit is 'packed' into a carton, then pallets might be irrelevant to finding 'unpacked' units.\n"
        "2. Extract ONLY the numbers mathematically necessary to answer the exact question asked.\n"
        "3. EXPLICITLY identify the distractor numbers and state: 'Ignoring [number] because [reason]'.\n"
        "4. Do the math in micro-steps. NEVER do large division or modulo in your head. "
        "To find A divided by B, write out: 'A / B is approx Q. Q * B = C. A - C = Remainder'.\n"
        "5. Calculate the final integer answer.\n\n"
        "Return ONLY a JSON object with EXACTLY two keys:\n"
        "- 'reasoning': A detailed string (>= 80 chars) explaining the logic, the micro-math, and ignored distractors.\n"
        "- 'answer': The final calculated integer.\n\n"
        f"PROBLEM:\n{problem}"
    )
    ...
    try:
        ...
        # Enforce the >80 character rule fallback
        if len(reasoning) < 80:
        ...
        # Safe fallback matching the original error handling style
