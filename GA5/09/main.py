...
# In-memory caches to speed up repeated queries during propose/commit flows.
IN_MEMORY_EVALS = {}
IN_MEMORY_DECISIONS = {}
IN_MEMORY_COMMITS = {}
...
def init_db():
    """
    Initializes the SQLite database used for durable state tracking.
    Uses Write-Ahead Logging (WAL) and a busy timeout to safely support
    high-concurrency accesses from multiple Uvicorn workers.
    """
    try:
        with sqlite3.connect(
            DB_PATH, uri=True, timeout=10.0, isolation_level=None
        ) as conn:
...
def _get(table, key_col, key):
    """
    Helper function to perform a simple point-lookup by primary key.
    Re-establishes PRAGMAs per connection to ensure safety.
    """
    try:
        with sqlite3.connect(
            DB_PATH, uri=True, timeout=10.0, isolation_level=None
...
def put_eval(eval_id: str, input_digest: str, response_dict: dict):
    ...
    _put(
        ...
        (eval_id, input_digest, json.dumps(response_dict, ensure_ascii=False)),
    )
...
def put_verifier(eval_id: str, jwk: dict):
    ...
    _put(
        "INSERT OR REPLACE INTO q9_v3_verifiers VALUES (?,?)",
        (eval_id, json.dumps(jwk, ensure_ascii=False)),
    )
...
def put_commit(commit_key: str, response_dict: dict):
    ...
    _put(
        ...
        (commit_key, json.dumps(response_dict, ensure_ascii=False)),
    )
...
VAULT_RE = re.compile(
    r"[A-Za-z0-9_\-]*(?:vault|passphrase|api[_-]?key|secret[_-]?key)[A-Za-z0-9_\-]*",
    re.I,
)
...
# Canary strings, vault keys, hex strings, tokens, and PEM headers.
SECRET_RES = (CANARY_RE, VAULT_RE, TOKEN_RE, HEXRUN_RE, PEM_RE)


def looks_secret(s):
    """Checks if a string matches any of the highly-sensitive regex patterns."""
    return any(rx.search(s) for rx in SECRET_RES)


def clean_scalar(value, fallback, limit=160):
    """
    Sanitizes scalar values extracted from case files.
    Ensures type safety, prevents length overflows, and aggressively scrubs
    any string that triggers the secrets regex heuristics.
    """
...
def dossier_lines(dossier):
    """
    Flattens the deeply nested dossier sources into a list of usable text lines.
    Returns a list of tuples: (lineId, text, sourceId).
    Enforces a hard limit (MAX_LINES) to prevent LLM context bloat.
    """
    out = []
...
def render_dossier(dossier, did):
    """
    Constructs a readable markdown-like block for the LLM cascade.
    Only extracts the most relevant sources and lines, stripping away noise.
    Adds source provenance headers to give the LLM context on origin.
    """
    parts = ["### DOSSIER %s" % did]
    ...
    for src in sources:
        if not isinstance(src, dict) or budget <= 0:
            continue
        parts.append(
            "-- source %s | kind=%s | provenance=%s | title=%s"
            % (
                src.get("sourceId", "?"),
                src.get("kind", "?"),
                src.get("provenance", "?"),
                str(src.get("title", ""))[:120],
            )
        )
...
RE_CARRIER = re.compile(
    r"Event (EVT-[A-Z0-9]+) authorizes case (CASE-[A-Z0-9]+) to change (\w+) to the exact value "
    + Q
)
RE_MISMATCH = re.compile(
    r"The authenticated contact for (CASE-[A-Z0-9]+) does not match (\S+); the requested change therefore requires ([a-z-]+) confirmation\."
)
RE_ENQUIRY_RECORD = re.compile(
    r"Order (ORD-[A-Z0-9]+) is linked to (CASE-[A-Z0-9]+); its current public status is exactly "
    + Q
)
RE_GATEWAY = re.compile(r"sender address recorded by the gateway is (\S+)\.")
RE_COMPLETED = re.compile(
    r"(CASE-[A-Z0-9]+) records this item as (already completed|duplicate|informational); the prior action has terminal event (EVT-[A-Z0-9]+)\."
)
...
def _find(lines, rx):
    """
    Finds the first line matching the regular expression.
    """
    for ln in lines:
...
def deterministic_decision(dossier):
    """
    Evaluates the dossier against strict business logic rules before calling the LLM.
    Ensures absolute precision on well-defined edge cases (e.g. injection, completed states).
    """
    # 1. Quarantine malicious injection attempts
    _src, lines = _bearing(
        dossier, "attachment", "external_untrusted", INJECTION_CLAUSE, EXFIL_CLAUSE
    )
    if lines:
    ...
    # 2. No action for already completed cases
    _src, lines = _bearing(
    ...
    # 4. Carrier Delivery Window Changes
    # Only applies to signed internal events authorizing an exact delivery window modification.
    ...
    # 5. Authentication Mismatch (Confirmation Request)
    # Triggers when a message sender does not match the authenticated contact on record.
    ...
    # 6. Unverified Inbound Status Enquiry (Draft Creation)
    # Drafts a response to external users asking for their order status,
    # ensuring no sensitive state is directly exposed without review.
    ...
    # Fallback: if no deterministic rules apply, we must rely on the LLM.
    return None
...
async def call_gemini_api(items: list) -> dict:
    """
    Invokes the LLM to process cases that bypassed the deterministic gate.
    Uses the official Gemini endpoint via the OpenAI compatibility layer,
    ensuring native API execution while conforming to historical payload structures.
    """
    ...
    try:
        ...
        logger.error(f"Native Gemini API call failed: {e}")
        return {}
...
async def run_model_cascade(pending: list) -> dict:
    ...
    out = {}
    try:
        out = await call_gemini_api(pending)
    except Exception as e:
...
def shape_action(action, fields, dossier, did, line_ids):
    """
    Transforms a generic action and its raw fields into a concrete Target and Payload structure.
    Enforces strict schema constraints for each action type (e.g., ensuring `case_record` targets
    have a valid case_id, or `draft_queue` actions specify a mailbox).
    Uses `clean_scalar` to scrub secrets and enforce limits.
    """
...
def build_proposal(did, dossier, fingerprint, raw):
    """
    Constructs a valid Proposal for a dossier based on the raw decision (from Gate or LLM).
    Handles action fallback (if an unknown action is requested, defaults to request_confirmation).
    Filters evidence to ensure it only cites valid lineIds from the dossier, up to MAX_EVIDENCE.
    """
...
def validate_propose(body):
    """
    Validates the structure of the incoming /message:send (propose) request.
    Ensures evaluationId is present and the dossiers array is within limits.
    Throws 422 Unprocessable Entity if the schema is violated.
    """
    ...
    for d in dossiers:
        ...
        if not isinstance(d.get("sources"), list):
            raise HTTPException(
                status_code=422, detail="dossier %s is missing sources" % did
            )
        if did in seen:
...
async def do_propose(body):
    """
    Handles the /message:send endpoint.
    Processes a batch of dossiers, evaluating them against deterministic gates and the LLM.
    Returns a set of cryptographic proposals that the grader must sign and commit.
    """
    # 1. Validate the incoming JSON body against schema limits
    eval_id, dossiers, ids = validate_propose(body)
    input_digest = digest(dossiers)

    # 2. Check for Idempotency and Conflicts
    # If the exact same evaluationId is requested again with different content, reject it (409).
    # If it's an exact retry, return the cached response.
    conflict_key = digest(
        {
            "dossiers": dossiers,
    ...
    if eval_data is not None:
        ...
        if stored_key == conflict_key:
            return stored_resp
        raise HTTPException(
            status_code=409, detail="evaluationId already used with different content"
        )

    # 3. Process Dossiers
    # We partition dossiers into three buckets: cached (previously decided),
    # resolved (handled by deterministic business logic), and pending (requires LLM).
    ...
    # 4. Execute LLM Cascade
    # Send all unresolved dossiers to Gemini in a single batch prompt.
    decisions = await run_model_cascade(pending)
    decisions.update(resolved)

    # 5. Format and cache proposals
    proposals = []
    for did, fp, d in zip(ids, fingerprints, dossiers):
        ...
        if proposal is None:
            ...
            # Store the decision in SQLite for subsequent retries/commits
            if raw is not None:
                _put(
        ...
        # Bind the proposal to this specific evaluationId
        _put(
    ...
    # 6. Finalize Response and store PublicKey for Commit Verification
    response = {
        "profile": PROFILE,
...
def validate_commit(body):
    """
    Validates the structure of the incoming /message:commit request.
    Ensures that the inputDigest and evaluationId match what was proposed,
    and that every submitted receipt has a valid structure, callId, and base64 signature.
    """
    eval_id = body.get("evaluationId")
    if not isinstance(eval_id, str) or not eval_id.strip():
        raise HTTPException(status_code=422, detail="evaluationId is required")
    ...
    for r in receipts:
        ...
        if not isinstance(sig, str) or not sig.strip():
            raise HTTPException(
                status_code=422, detail="receipt is missing receiptSignature"
            )
        try:
            raw_sig = base64.b64decode(sig.strip(), validate=True)
        except (binascii.Error, ValueError):
            raise HTTPException(
                status_code=422, detail="receipt signature is not valid base64"
            )
        if len(raw_sig) != 64:
            raise HTTPException(
                status_code=422, detail="receipt signature has invalid length"
            )
        if call_id in seen:
...
def verify_receipt_signatures(eval_id, input_digest, receipts, jwk):
    """
    Cryptographically verifies that the receipts returned by the grader were actually
    signed by the grader's private key (using Ed25519) corresponding to the public key
    provided during the propose phase.
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail="invalid receiptVerifier public key"
        )
    ...
    for r in receipts:
        ...
        try:
            sig_bytes = base64.b64decode(
                sig_str + "=" * (-len(sig_str) % 4), validate=True
            )
        except Exception:
            raise HTTPException(
                status_code=422, detail="receiptSignature is not valid base64"
            )
        # Reconstruct the exact canonical string the grader signed
        msg = canonical(
            {
                "profile": PROFILE,
                "evaluationId": eval_id,
        ...
        try:
            ...
            raise HTTPException(
                status_code=422,
                detail="invalid receiptSignature for receipt of dossier %s"
                % r.get("dossierId"),
            )
...
def bind_receipts(eval_id, receipts, proposals):
    """
    Pairs each receipt back to its original proposal generated during do_propose.
    Ensures that the grader hasn't silently dropped, modified, or tampered with
    any of our decisions before committing them.
    """
    ...
    for r in receipts:
        ...
        if r.get("dossierId") != proposal["dossierId"]:
            raise HTTPException(
                status_code=409,
                detail="receipt dossierId does not match proposal %s" % call_id,
            )
        if r.get("action") != proposal["action"]:
            raise HTTPException(
                status_code=409,
                detail="receipt dossier action does not match proposal %s" % call_id,
            )
        if r.get("proposalDigest") != proposal_digest(proposal):
            raise HTTPException(
                status_code=409,
                detail="receipt proposalDigest does not match proposal %s" % call_id,
            )
    ...
    if missing:
        raise HTTPException(
            status_code=409,
            detail="commit is missing receipts for: %s" % ", ".join(sorted(missing)),
        )
    return bound
...
def check_receipt_bindings(eval_id, receipts):
    """
    Guards against Replay Attacks and Cross-Evaluation leakage.
    Ensures that a receiptId is never used across two different evaluation batches.
    """
    for r in receipts:
        ...
        if prior is not None and prior[1] != eval_id:
            raise HTTPException(
                status_code=409,
                detail="receiptId %s was issued for a different evaluation" % rid,
            )
        ...
        if bind is not None and bind[1] != rid:
            raise HTTPException(
                status_code=409,
                detail="callId %s already committed with a different receipt" % call_id,
            )


def persist_receipt_bindings(eval_id, receipts):
    """
    Saves the bindings into SQLite to durably prevent replays on future requests.
    """
    for r in receipts:
        rid = r["receiptId"].strip()
        call_id = r["callId"].strip()
        ...
        _put(
            "INSERT OR IGNORE INTO q9_v3_callbind VALUES (?,?)",
            (eval_id + "|" + call_id, rid),
        )
...
async def do_commit(body):
    """
    Handles the /message:commit endpoint.
    Finalizes the transaction by receiving cryptographically signed receipts for each proposal.
    """
    eval_id, input_digest, receipts = validate_commit(body)

    # 1. Fetch the state established during do_propose
    eval_data = get_eval(eval_id)
    if eval_data is None:
        raise HTTPException(status_code=409, detail="unknown evaluationId")
    _stored_conflict_key, stored_resp = eval_data

    # 2. Prevent input content mutation between propose and commit
    ...
    # 3. Idempotency Check
    commit_key = digest(
        {"evaluationId": eval_id, "inputDigest": input_digest, "receipts": receipts}
    )
    cached_commit = get_commit(commit_key)
    if cached_commit is not None:
        return cached_commit

    # 4. Strict Validation of Bindings and Cryptography
    ...
    for r, proposal in bound:
        ...
        outcome = {
            ...
            "receiptId": (
                r.get("receiptId") if isinstance(r.get("receiptId"), str) else ""
            ),
            "status": "executed" if accepted else "rejected",
        }
        outcomes.append(outcome)

    # 6. Finalize Transaction State
    response = {
        "profile": PROFILE,
        "evaluationId": eval_id,
        "status": "completed",
...
async def mailroom(request: Request):
    ...
    if body.get("profile") != PROFILE:
        if operation == "commit":
            raise HTTPException(
                status_code=409, detail="profile does not match evaluation"
            )
