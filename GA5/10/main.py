    ...
    def get_route_handler(self):
        ...
        async def handler(request: Request):
            try:
                ...
                response = err(
                    400, "INVALID_ARGUMENT", "request failed schema validation"
                )
...
def load_task(task_id):
    """
    Retrieve a task and its principal owner from the SQLite database.
    This guarantees isolated retrieval where tasks are intrinsically linked
    to the principal (Bearer token) that created them.
    """
    with _db_lock:
        row = (
            db()
            .execute("SELECT doc, principal FROM q10_tasks WHERE task_id=?", (task_id,))
            .fetchone()
        )
    if not row:
        return None, None
...
def save_task(task, principal, batch_id):
    """
    Persist a task state to the SQLite database.
    Updates the state, doc payload, and timestamps in a thread-safe manner
    using the global database lock to prevent race conditions.
    """
    now = time.time()
    with _db_lock:
        c = db()
...
def check_headers(request, *, body=False):
    ...
    if who is None:
        return None, err(
            401, "UNAUTHENTICATED", "a Bearer token is required on every A2A route"
        )
    ...
    if body:
        ...
        if ctype != A2A_MEDIA_TYPE:
            return None, err(
                415, "UNSUPPORTED_MEDIA_TYPE", f"expected content type {A2A_MEDIA_TYPE}"
            )
    return who, None
...
def build_card(request: Request) -> dict:
    """Advertise a base URL exactly matching the request origin."""
    ...
    return {
        ...
        "defaultInputModes": [MODE_BATCH, MODE_RESULTS],
        "defaultOutputModes": [MODE_PROPOSALS, MODE_RECEIPTS],
        "securitySchemes": {
            "bearerAuth": {
                "type": "http",
                "scheme": "bearer",
...
INVOICE_RE = re.compile(
    r"\b(?:INV|INVOICE|BILL)[-/ ]?([A-Za-z0-9][A-Za-z0-9\-/]{2,})", re.I
)
REF_PATTERNS = [
...
HEURISTIC_SIGNALS = [
    ...
    (
        "open_exception",
        [
            r"materially conflict",
            r"records conflict",
            r"irreconcilable",
            r"contradict",
            r"does not (?:match|reconcile)",
            r"discrepanc",
        ],
    ),
    (
        "hold_invoice",
        [
            r"pending (?:verification|inspection|confirmation|clearance)",
    ...
    (
        "request_approval",
        [
            r"exceeds .{0,40}(?:limit|authority|threshold)",
            r"outside .{0,30}(?:delegated )?authority",
            r"requires .{0,20}approval",
            r"above the .{0,30}threshold",
        ],
    ),
]
NEGATORS = re.compile(
    ...
    r"for illustration|in an earlier case",
    re.I,
)
...
def _all_text(pkg):
    return "\n".join(
        (d.get("name", "") + "\n" + d.get("text", "")) for d in _documents(pkg)
    )
...
def heuristic_facts(pkg, text):
    ...
    if m:
        ...
        if "." in num:
            ...
            amount = (
                int(whole + (frac + "0" * exponent)[:exponent])
                if exponent
                else int(whole)
            )
        else:
...
def compact_task(task):
    ...
    return {
        ...
        "history": [],
        "artifacts": [
            dict(art, parts=[part_descriptor(p) for p in art.get("parts") or []])
...
def fit(payload, task_key=None):
    ...
    if history:
        task["history"] = [
            dict(m, parts=[part_descriptor(p) for p in m.get("parts") or []])
            for m in history
        ]
        if _size(payload) <= MAX_BODY:
            return payload
        ...
        if _size(payload) <= MAX_BODY:
            return payload
    task["artifacts"] = [
        dict(a, parts=[part_descriptor(p) for p in a.get("parts") or []])
...
def find_part(message, media_type):
    for part in message.get("parts") or []:
        if not isinstance(part, dict):
            continue
        mt = (
            part.get("mediaType") or (part.get("metadata") or {}).get("mediaType") or ""
        )
        if mt == media_type:
            return part
    return None
...
async def message_send(request: Request):
    ...
    The primary A2A entrypoint for submitting batch packages.
    Parses incoming messages, validates schema constraints, handles replay deduplication,
    and invokes the task creation logic asynchronously.
    """
...
async def start_task(who, message, message_id, fingerprint):
    ...
    if not isinstance(packages, list) or not packages:
        return err(400, "INVALID_ARGUMENT", "packages must be a non-empty array")
    if not all(isinstance(p, dict) for p in packages):
        return err(400, "INVALID_ARGUMENT", "each package must be an object")
    ...
    if len(set(ids)) != len(ids):
        return err(400, "INVALID_ARGUMENT", "duplicate packageId in batch")
    ...
    if existing and owner == who:
        with _db_lock:
            c = db()
            c.execute(
                "INSERT OR REPLACE INTO q10_msgs(principal,message_id,fingerprint,task_id)"
                " VALUES(?,?,?,?)",
...
async def continue_task(who, message, message_id, fingerprint):
    ...
    if state in TERMINAL:
        with _db_lock:
            row = (
                db()
                .execute("SELECT results_fp FROM q10_final WHERE task_id=?", (task_id,))
                .fetchone()
            )
        if state == COMPLETED and row and row["results_fp"] == results_fp:
            return task_envelope(task)
        return err(409, "TASK_TERMINAL", f"task is already in {state} and is immutable")
    ...
    for res in results:
        ...
        if outcome == "ACCEPTED" and not (isinstance(nonce, str) and nonce.strip()):
            return err(
                400, "INVALID_ARGUMENT", "an ACCEPTED result requires a receiptNonce"
            )
        ...
        executions.append(
            {
                "receiptId": "rcpt_"
                + sha("q10-rcpt", task_id, prop["actionId"], str(nonce))[:16],
                ...
                "outcome": outcome,
                "status": (
                    "executed" if outcome in ("ACCEPTED", "EXECUTED") else "rejected"
                ),
                "facts": prop["facts"],
    ...
    with _db_lock:
        ...
        if state in TERMINAL:
            row = c.execute(
                "SELECT results_fp FROM q10_final WHERE task_id=?", (task_id,)
            ).fetchone()
            if state == COMPLETED and row and row["results_fp"] == results_fp:
                return task_envelope(fresh)
            return err(
                409, "TASK_TERMINAL", f"task is already in {state} and is immutable"
            )
        ...
        now = time.time()
        c.execute(
            "UPDATE q10_tasks SET state=?, doc=?, updated=? WHERE task_id=?",
            (COMPLETED, json.dumps(fresh), now, task_id),
        )
        c.execute(
            "INSERT OR REPLACE INTO q10_final(task_id,results_fp) VALUES(?,?)",
            (task_id, results_fp),
        )
        c.commit()
    return task_envelope(fresh)
...
async def list_tasks(request: Request):
    ...
    with _db_lock:
        rows = (
            db()
            .execute(
                "SELECT doc FROM q10_tasks WHERE principal=? ORDER BY created", (who,)
            )
            .fetchall()
        )
...
async def cancel_task(task_id: str, request: Request):
    ...
    Cancels an active task if it is not already in a TERMINAL state.
    Strictly verifies ownership to prevent unauthorized interference.
    """
    who, bad = check_headers(request)
    if bad:
        return bad
    with _db_lock:
        ...
        if state in TERMINAL:
            ...
            return err(
                409, "TASK_NOT_CANCELABLE", f"task is already in terminal state {state}"
            )
